...
const burst_signal_t mcgenSineTable256[SIN_LENGTH_TABLE+1] = 
...
burst_signal_t mcsinPIxLUT(burst_signal_t Value)
{ 
    if (Value >= 0)
    {
			
        return ( Value > PI_DIV_2 ? -mcgenSineTable256[(2 * SIN_LENGTH_TABLE- \
        (Value >> SIN_INDEX_SHIFT) - 1)] : (-mcgenSineTable256[(Value >> SIN_INDEX_SHIFT)]));
	}
	else
	{
        return ( Value <= -PI_DIV_2 ? mcgenSineTable256[(2 * SIN_LENGTH_TABLE- \
        ( (-Value) >> SIN_INDEX_SHIFT))] : mcgenSineTable256[(( (-Value) >> SIN_INDEX_SHIFT))] );
    }
}

burst_signal_t burst_sin(burst_signal_t phi){
	return mcsinPIxLUT(phi);
}
burst_signal_t burst_cos(burst_signal_t phi){
	return mcsinPIxLUT( phi + PI_DIV_2 );
}

#define TAN_LENGTH_TABLE 64
const burst_signal_t tableAtan64[TAN_LENGTH_TABLE] = 
{ 
...
burst_signal_t _atan( burst_usignal_t x){
	int n = x>>10;
...
burst_signal_t burst_atan2(burst_signal_t y,burst_signal_t x){
	...
	if (y==0){
	    ...
	    if (x==0){
			...
			burst_signal_t  ret;
			burst_usignal_t  abs_x = BURST_ABS(x);
			burst_usignal_t  abs_y = BURST_ABS(y);
			if ( abs_y<=abs_x){
				ret = _atan(((burst_usignal_t)(65536L * abs_y) / abs_x));
			}  else {
				ret = PI_DIV_2 - _atan((burst_usignal_t)((65536L * abs_x) / abs_y) );
			}
			if (x<0){
				if (y<0) {
					ret = -PI + ret;
